...
elastic_index = "stocks-prediction"
...
def clearData(x):
    return x[0][1].replace("'",'"').replace('u"','"')
...
def predict(dataframe, something, time):
    ...
    predictions.show(truncate=False) 
    return {
        'prediction' : getItemFromDataFrame(predictions, 'prediction', 0),
        'rsquared_error' : results.r2
    }
...
def handler(message):
    ...
    if(records != []):
        try:
            ...
            last_price_prediction = predict(df, 'LastPrice', input_dataframe)

            recordsJSON = json.loads(records[0])
            response = {
                'type': 'predicted_stocks',
                '@timestamp': str_plus_n,
                'Name': str(recordsJSON['Name']),
                'Country': str(recordsJSON['Country']),
                'Symbol': str(recordsJSON['Symbol']),
                'WebUrl': str(recordsJSON['WebUrl']),
                'MarketCap': str(recordsJSON['MarketCap']),
                'LastPricePlus5': last_price_prediction['prediction'],
                'RsquaredError': last_price_prediction['rsquared_error']
            }
            print response
